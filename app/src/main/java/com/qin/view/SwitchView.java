...
public class SwitchView extends View {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int heightSize = (int) (widthSize * 0.40f);
	...
	protected void onDraw(Canvas canvas) {
		...
		paint.setStyle(Style.FILL);
		paint.setColor(isOn ? 0xffffffff:0xff4bd763);
		if (!isOn) {
			String text="农历";
			FontMetrics fontMetrics = paint.getFontMetrics();
			float x = sWidth-bWidth- paint.measureText(text);
			canvas.drawText(text, bWidth+x/2-bStrokeWidth, sHeight / 2
					+ (fontMetrics.descent - fontMetrics.ascent) / 2
					- fontMetrics.descent, paint);
		}else{
			String text="公历";
			FontMetrics fontMetrics = paint.getFontMetrics();
			float x = sWidth-bWidth- paint.measureText(text);
			canvas.drawText(text,-x/2-paint.measureText(text)+bStrokeWidth, sHeight / 2
					+ (fontMetrics.descent - fontMetrics.ascent) / 2
					- fontMetrics.descent, paint);
		}
		canvas.restore();
