...
public class LunarCalendar {
    ...
    private static final  String chineseNumberMonth[] = { "正", "二", "三", "四", "五", "六", "七",
            "八", "九", "十", "冬", "腊" };
    private static final  String chineseNumberDay[] = { "初一", "初二", "初三", "初四", "初五", "初六",
        ...
        public static final int[] lunarToSolar(int year, int month, int monthDay,
                ...
                solarInfo[0] = year;
            solarInfo[3] = -1;//无用，只是用来占位，保持和农历一致
                return solarInfo;
        }
   ...
   /**
	 ...
	 */
	 public static final int leapDays(int y) {
		if (leapMonth(y) != 0) {
			if ((LUNAR_INFO[y - MIN_YEAR] & 0x10000) != 0)
				return 30;
			else
				return 29;
		} else
     ...
     * @param month
     * @param monthDay
     * @return 返回公历日期对应的农历日期String
     */
     public  static final String solarToLunarText(int year, int month, int monthDay){
         String dateText="";
         int date[]=solarToLunar(year,month,monthDay);
         dateText=date[0]+"年";
         if (date[3]==1){
             dateText+="润";
         }
         dateText+=chineseNumberMonth[date[1]-1]+"月"+chineseNumberDay[date[2]-1];
          return dateText;
     }
    /**
     * 将农历日期转成中文
     ...
     * @return 返回农历日期中文
     */
     public  static final String lunartoChineseText(int year, int month, int monthDay,int isLeap){
         String dateText="";
         dateText=year+"年";
         if (isLeap==1){
             dateText+="润";
         }
         dateText+=chineseNumberMonth[month-1]+"月"+chineseNumberDay[monthDay-1];
         return dateText;
     }
